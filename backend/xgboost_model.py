...
class XGBoostModel:
    """
    Version « sans xgboost » du modèle, utilisant une régression logistique maison.
    L'interface (attributs, méthodes) reste la même que dans xgboost_model.py.
    """

    def __init__(self):
        # On réutilise la classe existante de regression_logistique.py
        self.model = LogisticRegressionModel(lr=0.05, n_iter=2000, reg_lambda=0.01)
    ...
    def train_and_evaluate(self, X_train, X_test, y_train, y_test):
        ...
        def factory():
            return LogisticRegressionModel(lr=self.model.lr, n_iter=self.model.n_iter, reg_lambda=self.model.reg_lambda)
        ...
        print(f"   Log-vraisemblance Train/Test: {train_ll:.6f} / {test_ll:.6f}")
        print(f"   Log Loss Train/Test: {train_cost:.6f} / {test_cost:.6f}")
...
def main():
    ...
    results = model.train_and_evaluate(
        data_prep.X_train,
        data_prep.X_test,
        data_prep.y_train_class,
        data_prep.y_test_class,
    )

    # Visualisation avec évolution du coût
    ...
    colors = plt.cm.cool(np.linspace(0, 1, len(feature_names)))
    axes[1].barh(feature_names, results["feature_importance"], color=colors)
    axes[1].set_xlabel("Importance")
    axes[1].set_title("Importance des Features - XGBoost (Logistique)")
    ...
    plt.tight_layout()
    plt.savefig("xgboost_analyse.png", dpi=200)
    plt.show()
